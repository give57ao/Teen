...
<head>
<meta charset="UTF-8">
<title>TeenCoder 내가 스크랩한 게시글</title>
...
<body>
	...
	<div id="contents">
		<div id="contents_title">
			<h1>내가 스크랩한 게시글</h1>
			<h2>내가 스크랩한 게시글 목록을 확인할 수 있습니다.</h2>
		</div>
		<div id="contents_detail">
			<div id="contents_wrap">
				...
				<ul id="contents_category">
					...
					<li><a href="/teen/scrap/scrapList" class="select">스크랩</a></li>
				</ul>
				<!-- Board -->
				<div id="board_list">
					<!-- Title -->
					<div id="board_list_title">
						...
						<ul id="board_list_menu" class="side_menu">
							<li><a href="scrapList?sort=recent&search=${paging.search}&keyword=${paging.keyword}">최신순</a></li>
							<li><a href="scrapList?sort=view&search=${paging.search}&keyword=${paging.keyword}">조회순</a></li>
							<li><a href="scrapList?sort=recommend&search=${paging.search}&keyword=${paging.keyword}">추천순</a></li>
							<li><a href="scrapList?sort=comment&search=${paging.search}&keyword=${paging.keyword}">댓글순</a></li>
						</ul>
					</div>
					<!-- Search -->
					...
					<div id="board_list_row">
						...
						<c:choose>
							<c:when test="${paging.total > 0}">
								<c:forEach items="${scrapList}" var="board" varStatus="status">
									<div class="row">
										<div class="row_info">
										...
										<div class="row_title">
											...
											<input type="button" value="삭제" class="btn_com btn_del_list" onClick="deleteScrap(${board.scrap_no})">
										</div>
									</div>
									<hr>
								</c:forEach>
					...
					<div id="board_list_pagination">
						<ul>
							...
							<c:if test="${paging.nowPage != 1}">
								<li><a class="prev" href="/teen/scrap/scrapList?sort=${sort}&search=${paging.search}&keyword=${paging.keyword}&nowPage=${paging.nowPage - 1}&cntPerPage=${paging.cntPerPage}"></a></li>
							</c:if>
							<!-- 페이징 숫자가 나오는 부분 -->
							<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="p">
								<c:choose>
									...
									<c:when test="${p != paging.nowPage}">
										<li><a href="/teen/scrap/scrapList?sort=${sort}&search=${paging.search}&keyword=${paging.keyword}&nowPage=${p}&cntPerPage=${paging.cntPerPage}">${p}</a></li>
									</c:when>
								</c:choose>
							</c:forEach>
							<!-- 오른쪽 버튼 -->
							<c:if test="${paging.startPage < paging.endPage}">
								<li><a class="next" href="/teen/scrap/scrapList?sort=${sort}&search=${paging.search}&keyword=${paging.keyword}&nowPage=${paging.nowPage + 1}&cntPerPage=${paging.cntPerPage}"></a></li>
							</c:if>
						</ul>
					</div>
				</div>
	...
	<!-- JS -->
	<script type="text/javascript" src="/teen/resources/js/member/myPage/myScrap.js"></script>
</body>
</html>